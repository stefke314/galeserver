...
public class VerticalFlowLayout implements LayoutManager, java.io.Serializable {
			...
			for (int i = rowStart; i < rowEnd; i++) {
				...
				if (m.isVisible()) {
					if (ltr) {
						...
						m.setLocation(x + (width - m.getWidth()) / 2,
	...
	public void layoutContainer(Container target) {
		synchronized (target.getTreeLock()) {
			...
			for (int i = 0; i < nmembers; i++) {
				...
				if (m.isVisible()) {
					...
					if ((y == 0) || ((y + d.height) <= maxheight)) {
						...
						moveComponents(target, x, insets.top + vgap, colw,
								maxheight - y, start, i, ltr);
						y = d.height;
						x += hgap + colw;
						colw = d.width;
						start = i;
			...
			moveComponents(target, x, insets.top + vgap, colw, maxheight - y,
					start, nmembers, ltr);
		}
	}
