...
public class TabbedPaneUI extends BasicTabbedPaneUI implements SwingConstants {
	...
	protected void paintTab(Graphics g, int tabPlacement, Rectangle[] rects,
		...
		paintTabBackground(g, tabPlacement, tabIndex, r.x, r.y, r.width + 2,
				r.height + 2, isSelected);
		...
		if (tabPlacement != LEFT) {
			...
			if (isSelected && drawThickBorder)
				g.drawLine(r.x + r.width - 1, r.y, r.x + r.width - 1, r.y
						+ r.height + 1);
		}
		if (tabPlacement != TOP) {
			g.drawLine(r.x, r.y + r.height, r.x + r.width, r.y + r.height);
			if (isSelected && drawThickBorder)
				g.drawLine(r.x, r.y + r.height - 1, r.x + r.width, r.y
						+ r.height - 1);
		}
		...
		layoutLabel(tabPlacement, metrics, tabIndex, title, icon, r, iconRect,
				textRect, isSelected);
	...
	protected void paintContentBorderTopEdge(Graphics g, int tabPlacement,
		...
		if (tabPlacement != TOP || selectedIndex < 0
			...
			if (selRect.x + selRect.width < x + w - 2) {
				...
				if (drawThickBorder)
					g.drawLine(selRect.x + selRect.width - 1, y + 1, x + w - 2,
							y + 1);
			} else {
				g.setColor(shadow);
	...
	protected void paintContentBorderLeftEdge(Graphics g, int tabPlacement,
		...
		if (tabPlacement != LEFT || selectedIndex < 0
			...
			if (selRect.y + selRect.height < y + h - 2) {
				...
				if (drawThickBorder)
					g.drawLine(x + 1, selRect.y + selRect.height, x + 1, y + h
							- 2);
			}
		}
	}
	...
	protected void paintContentBorderBottomEdge(Graphics g, int tabPlacement,
		...
		if (tabPlacement != BOTTOM || selectedIndex < 0 || (selRect.y - 1 > h)
			...
			if (selRect.x + selRect.width < x + w - 2) {
				...
				if (drawThickBorder)
					g.drawLine(selRect.x + selRect.width, y + h - 2, x + w - 1,
							y + h - 2);
			}
		}
	...
	protected Rectangle getTabBounds(int tabIndex, Rectangle dest) {
		...
		if (scrollableTabLayoutEnabled()) { // SCROLL_TAB_LAYOUT
											// Need to translate coordinates
											// based on viewport location &
											// view position
	...
	public class TabbedPaneLayout implements LayoutManager {
		...
		protected void calculateTabRects(int tabPlacement, int tabCount) {
			...
			boolean leftToRight = TonicUtils.isLeftToRight(tabPane);
	...
	private class ScrollableTabPanel extends JPanel implements UIResource {
		...
		public void paintComponent(Graphics g) {
			...
			TabbedPaneUI.this.paintTabArea(g, tabPane.getTabPlacement(),
