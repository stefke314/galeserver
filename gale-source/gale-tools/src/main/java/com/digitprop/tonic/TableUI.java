...
public class TableUI extends BasicTableUI {
	...
	InputMap getMyInputMap(int condition) {
		if (condition == JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT) {
			InputMap keyMap = (InputMap) UIManager
					.get("Table.ancestorInputMap");
			InputMap rtlKeyMap;

			if (table.getComponentOrientation().isLeftToRight()
					|| ((rtlKeyMap = (InputMap) UIManager
							.get("Table.ancestorInputMap.RightToLeft")) == null)) {
				return keyMap;
			} else {
	...
	/**
	 * Paints a representation of the <code>table</code> instance that was set
	 * in installUI().
	 */
	public void paint(Graphics g, JComponent c) {
		...
		paintGrid(g, rMin, rMax, cMin, cMax, c.getInsets().left,
				c.getInsets().top, drawLastLine);

		// Paint the cells.
		paintCells(g, rMin, rMax, cMin, cMax, c.getInsets().left,
				c.getInsets().top);
	}
	...
	private void paintGrid(Graphics g, int rMin, int rMax, int cMin, int cMax,
			int dx, int dy, boolean drawLastLine) {
		...
		if (table.getShowHorizontalLines()) {
			...
			for (int row = rMin; row <= rMax; row++) {
				...
				if (row + 1 < table.getRowCount())
					g.drawLine(damagedArea.x + dx, y - 1 + dy, tableWidth - 1
							+ dx, y - 1 + dy);
			}

			if (drawLastLine) {
				g.drawLine(damagedArea.x + dx, y - 1 + dy, tableWidth - 1 + dx,
						y - 1 + dy);
			}
		}
			...
			if (table.getComponentOrientation().isLeftToRight()) {
				...
				for (int column = cMin; column <= cMax; column++) {
					...
					if (column + 1 < table.getColumnCount())
						g.drawLine(x - 1 + dx, 0 + dy, x - 1 + dx, tableHeight
								+ dy);
				}
			} else {
	...
	private void paintCell(Graphics g, Rectangle cellRect, int row, int column,
		...
		if (table.isEditing() && table.getEditingRow() == row
			...
			rendererPane.paintComponent(g, component, table, cellRect.x + dx,
					cellRect.y + dy, cellRect.width - 1, cellRect.height - 1,
					true);
		}
	}
	...
	private static class NavigationalAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (!inSelection) {
				...
				if (!extend) {
					table.changeSelection(anchorRow, anchorColumn, false,
							extend);
				} else {
	...
	private static class PagingAction extends NavigationalAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (toLimit) {
				...
				ListSelectionModel sm = (vertically) ? table
						.getSelectionModel() : table.getColumnModel()
						.getSelectionModel();

				int start = (extend) ? sm.getLeadSelectionIndex() : sm
	...
	public class MouseInputHandler implements MouseInputListener {
		...
		private boolean shouldIgnore(MouseEvent e) {
			return e.isConsumed()
					|| (!(SwingUtilities.isLeftMouseButton(e) && table
							.isEnabled()));
		}
		...
		void adjustFocusAndSelection(MouseEvent e) {
			...
			if (editor == null || editor.shouldSelectCell(e)) {
				boolean adjusting = (e.getID() == MouseEvent.MOUSE_PRESSED) ? true
						: false;
	...
	static class TableDragGestureRecognizer extends TonicDragGestureRecognizer {
		 ...
		 * </ul>
		 * <p>
		 * This is implemented to perform the superclass behavior followed by a
		 * check if the dragEnabled property is set and if the location picked
		 * is selected.
		 */
	...
	/**
	 * A DropTargetListener to extend the default Swing handling of drop
	 * operations by moving the tree selection to the nearest location to the
	 * mouse pointer. Also adds autoscroll capability.
	 */
	...
	static class TableTransferHandler extends TransferHandler implements
		...
		protected Transferable createTransferable(JComponent c) {
			if (c instanceof JTable) {
				...
				return new TonicTransferable(plainBuf.toString(),
						htmlBuf.toString());
			}
	...
	/**
	 * PropertyChangeListener for the table. Updates the appropriate varaible,
	 * or TreeState, based on what changes.
	 */
