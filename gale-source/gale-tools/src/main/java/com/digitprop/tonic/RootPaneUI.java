	...
	private class MouseInputHandler implements MouseInputListener {
		...
		public void mouseClicked(MouseEvent ev) {
			...
			if (prevMousePos != null) {
				int deltaX = Math.abs(prevMousePos.x - ev.getX()
						- root.getLocationOnScreen().x);
				int deltaY = Math.abs(prevMousePos.y - ev.getY()
						- root.getLocationOnScreen().y);
				System.out.println("Mouse pos: " + ev.getPoint()
						+ ", prev Pos=" + prevMousePos + ", dx=" + deltaX
						+ ", dy=" + deltaY);
