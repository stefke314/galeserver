...
/**
 * The UI delegate for JFileChooser instances. This is basically identical to
 * the Metal file chooser, except that it prevents the renaming of files at
 * simple double clicks, and that is displays the OS specific icons for files
 * types, instead of generic icons.
 * 
...
public class FileChooserUI extends BasicFileChooserUI {
	...
	public void installComponents(JFileChooser fc) {
		...
		fileNameTextField = new JTextField() {
			private static final long serialVersionUID = 5536995708978566026L;
	...
	protected ActionMap createMyActionMap() {
		AbstractAction escAction = new AbstractAction() {
			private static final long serialVersionUID = -639989813968543756L;
	...
	class DetailsTableModel extends AbstractTableModel implements
			ListDataListener {
		private static final long serialVersionUID = 402936233638910442L;
		String[] columnNames = { fileNameHeaderText, fileSizeHeaderText,
				fileTypeHeaderText, fileDateHeaderText, fileAttrHeaderText };
	...
	class DetailsTableCellRenderer extends DefaultTableCellRenderer {
		...
		public void setValue(Object value) {
			...
			if (value instanceof File) {
				...
				setText(fileName);
				Icon icon = FileChooserUI.getIcon(file, fileIcon,
						getFileChooser().getFileSystemView());
				setIcon(icon);
						...
						if (objects != null) {
							if (objects.length == 1
								...
								for (int i = 0; i < objects.length; i++) {
									...
									if ((chooser.isFileSelectionEnabled() && f
											.isFile())
											|| (chooser
													.isDirectorySelectionEnabled()
													&& fsv.isFileSystem(f) && f
														.isDirectory())) {
										files[j++] = f;
									}
								}
	...
	protected class FileRenderer extends DefaultListCellRenderer {
		public Component getListCellRendererComponent(JList list, Object value,
			...
			Icon icon = FileChooserUI.getIcon(file, fileIcon, getFileChooser()
					.getFileSystemView());
			setIcon(icon);
	 ...
	 * 
	 * @param f
	 *            The file for which to return the icon
	 * @param fileIcon
	 *            The default file icon
	 * @param fsv
	 *            The file system view from which we get the system-specific
	 *            file icons
	 * 
	 * @return The icon for the specified file, or the default icon, if the file
	 *         system view did not provide a suitable icon.
	 */
	public static Icon getIcon(File f, Icon fileIcon, FileSystemView fsv) {
		...
		icon = fileIcon;
		if (f != null) {
			if (f.isDirectory() && !fsv.isDrive(f) && !fsv.isFileSystemRoot(f)
					&& !fsv.isComputerNode(f) && !fsv.isFloppyDrive(f)
					&& !fsv.isRoot(f))
	...
	/**
	 * Ensures that the specified file in the specified JFileChooser is visible
	 * in the file list.
	 */
	...
	/**
	 * Updates the display for the current directory, by rescanning it and
	 * updating the files contained in the directory.
	 */
	...
	/**
	 * Returns the file name of the selected file, or null if the user has not
	 * entered or selected a file.
	 */
	...
	class DirectoryComboBoxRenderer extends DefaultListCellRenderer {
		...
		public Component getListCellRendererComponent(JList list, Object value,
			...
			Icon icon = FileChooserUI.getIcon(directory, fileIcon,
					getFileChooser().getFileSystemView());
			ii.icon = icon;
		...
		private void addItem(File directory) {
			...
			for (int i = 0; i < pathCount; i++) {
				...
				if (directories.contains(f)) {
					...
					for (int j = i - 1; j >= 0; j--) {
						directories.insertElementAt(path.get(j), topIndex + i
								- j);
					}
					break;
				}
