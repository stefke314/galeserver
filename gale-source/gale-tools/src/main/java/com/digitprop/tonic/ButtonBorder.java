...
/**
 * Represents the standard border for AbstractButtons. This border determines
 * whether the underlying button touches the edge of the parent component, and
 * draws an edge only where the button is not touching the parent's edge.
 * <p>
 * 
...
public class ButtonBorder extends IntelligentLineBorder {
	...
	/**
	 * Returns the insets of this button. They depend upon whether the button
	 * touches the parent's edges or not .
	 */
	...
	public void paintBorder(Component c, Graphics g, int x, int y, int width,
		...
		if (c instanceof AbstractButton) {
			...
			if (!b.getModel().isPressed()
					&& !((c instanceof JToggleButton) && ((JToggleButton) c)
							.isSelected())) {
				right++;
				bottom++;
			}
			...
			boolean drawPressed = (b.getModel().isPressed() || ((c instanceof JToggleButton) && ((JToggleButton) c)
					.isSelected()));
			...
			if (!isTouchingBottomEdge(c)) {
				if (drawPressed)
					...
					g.drawLine(x + 1, y + height - 2, x + width - 1, y + height
							- 2);
			}
