...
public class ScrollButton extends BasicArrowButton {
	 ...
	 * 
	 * @param direction
	 *            The direction into which the arrow on the button is to point.
	 *            This must be a SwingConstants constant.
	 * @param width
	 *            The width of the button
	 * @param freeStanding
	 *            If true, the button is freestanding
	 */
	...
	/**
	 * Sets whether the button is free standing, i.e. does not touch any edge of
	 * the parent component.
	 */
	...
	public void paint(Graphics g) {
		...
		if (isPressed) {
			...
			g.drawRect(shadowRect.x, shadowRect.y, shadowRect.width - 1,
					shadowRect.height - 1);

			g.setColor(UIManager.getColor("Button.shadow"));
			g.drawLine(shadowRect.x, shadowRect.y, shadowRect.x
					+ shadowRect.width - 1, shadowRect.y);
			g.drawLine(shadowRect.x, shadowRect.y, shadowRect.x, shadowRect.y
					+ shadowRect.height - 1);
		}
		...
		if (icon != null)
			icon.paintIcon(this, g, getWidth() / 2 - icon.getIconWidth() / 2
					+ delta, getHeight() / 2 - icon.getIconHeight() / 2 + delta);
	}
