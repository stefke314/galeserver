...
public class MenuItemUI extends BasicMenuItemUI {
	...
	/**
	 * Creates and returns a property change listener for the specified
	 * component
	 */
	...
	/**
	 * Updates the accelerator bindings for the associated menu item (in the
	 * case that they have changed).
	 */
	 ...
	 * 
	 * @param c
	 *            The component for which to return the preferred size
	 * @param checkIcon
	 *            The check icon (if any)
	 * @param arrowIcon
	 *            The arrow icon (if any)
	 * @param defaultTextIconGap
	 *            The default gap in pixels between icon and text
	 */
	protected Dimension getPreferredMenuItemSize(JComponent c, Icon checkIcon,
		...
		layoutMenuItem(fm, text, fmAccel, acceleratorText, icon, checkIcon,
				arrowIcon, b.getVerticalAlignment(),
				b.getHorizontalAlignment(), b.getVerticalTextPosition(),
				b.getHorizontalTextPosition(), viewRect, iconRect, textRect,
				acceleratorRect, checkIconRect, arrowIconRect, text == null ? 0
						: defaultTextIconGap, defaultTextIconGap, maxTextWidth,
				maxAccWidth, arrowUsed, iconUsed);
	...
	/**
	 * Updates the visual appearance of the specified component. Invokes
	 * paint().
	 * 
	...
	/**
	 * Returns true if any icon is used in this menu item or other menu items at
	 * the same hierarchy level in the menu tree.
	 */
		...
		if (p != null) {
			if (p instanceof JMenu) {
				...
				for (int i = 0; i < menu.getMenuComponentCount(); i++) {
					...
					if (currItem.getIcon() != null
							|| (currItem instanceof JCheckBoxMenuItem)
	...
	/**
	 * Returns true, if any arrow is used in this item or any item on the same
	 * hierarchy level in the menu tree.
	 */
	...
	/**
	 * Calculates and returns the width of the text displayed on this menu item.
	 */
			...
			if (p instanceof JMenu) {
				...
				for (int i = 0; i < menu.getMenuComponentCount(); i++) {
					...
					maxTextWidth = Math.max(maxTextWidth,
							fm.stringWidth(currItem.getText()));
				}
			} else if (p instanceof JPopupMenu) {
				JPopupMenu menu = (JPopupMenu) p;
				for (int i = 0; i < menu.getComponentCount(); i++) {
	...
	/**
	 * Returns the width of the accelerator text displayed on the specified menu
	 * item.
	 */
	private int calcAcceleratorWidth(JMenuItem item) {
		...
		int maxTextWidth = fm.stringWidth(getAcceleratorText(item
				.getAccelerator()));
		...
		if (p != null) {
			if (p instanceof JMenu) {
				...
				for (int i = 0; i < menu.getMenuComponentCount(); i++) {
					JMenuItem currItem = (JMenuItem) menu.getMenuComponent(i);
					maxTextWidth = Math.max(maxTextWidth, fm
							.stringWidth(getAcceleratorText(item
									.getAccelerator())));
				}
			} else if (p instanceof JPopupMenu) {
				JPopupMenu menu = (JPopupMenu) p;
				for (int i = 0; i < menu.getComponentCount(); i++) {
					...
					if (o instanceof JMenuItem) {
						JMenuItem currItem = (JMenuItem) o;
						maxTextWidth = Math.max(maxTextWidth, fm
								.stringWidth(getAcceleratorText(currItem
										.getAccelerator())));
					}
				}
			}
	...
	public void paint(Graphics g, JComponent c) {
		...
		paintMenuItem(g, c, checkIcon, arrowIcon, selectionBackground,
				selectionForeground, defaultTextIconGap, maxTextWidth,
				maxAccWidth, arrowUsed, iconUsed);
	}
	 ...
	 * @param c
	 *            The component which to paint
	 * @param checkIcon
	 *            The check mark icon (if any)
	 * @param arrowIcon
	 *            The arrow icon (if any)
	 * @param background
	 ...
	 * @param defaultTextIconGap
	 *            The default width in pixels between the icon and the text
	 * @param maxTextWidth
	 *            The maximum width in pixels of the text
	 * @param maxAccWidth
	 *            The maximum width in pixels of the accelerator
	 * @param arrowUsed
	 *            If true, the arrow will be painted
	 * @param iconUsed
	 *            If true, the icon will be painted
	 */
	protected void paintMenuItem(Graphics g, JComponent c, Icon checkIcon,
			Icon arrowIcon, Color background, Color foreground,
			int defaultTextIconGap, int maxTextWidth, int maxAccWidth,
			boolean arrowUsed, boolean iconUsed) {
		JMenuItem b = (JMenuItem) c;
		...
		// && !(menuItem instanceof JMenu && ((JMenu)
		// menuItem).isTopLevelMenu()))
		// {
		// viewRect.x += 16;
		// }
		...
		String text = layoutMenuItem(fm, b.getText(), fmAccel, acceleratorText,
				b.getIcon(), checkIcon, arrowIcon, b.getVerticalAlignment(),
				b.getHorizontalAlignment(), b.getVerticalTextPosition(),
				b.getHorizontalTextPosition(), viewRect, iconRect, textRect,
				acceleratorRect, checkIconRect, arrowIconRect,
				b.getText() == null ? 0 : defaultTextIconGap,
				defaultTextIconGap, maxTextWidth, maxAccWidth, arrowUsed,
				iconUsed);
		...
		if (checkIcon != null) {
			...
			if (useCheckAndArrow()) {
				if (b.isSelected())
					...
					uncheckIcon.paintIcon(c, g, checkIconRect.x,
							checkIconRect.y);
			}
			g.setColor(holdc);
		}
			...
			if (useCheckAndArrow()) {
				if (model.isArmed()
					...
					if (invArrowIcon != null)
						invArrowIcon.paintIcon(c, g, arrowIconRect.x,
								arrowIconRect.y);
				} else {
					if (arrowIcon != null)
	...
	private String layoutMenuItem(FontMetrics fm, String text,
			FontMetrics fmAccel, String acceleratorText, Icon icon,
			...
			int menuItemGap, int maxTextWidth, int maxAccWidth,
			boolean arrowUsed, boolean iconUsed) {
		...
		acceleratorRect.y = (viewRect.height - fmAccel.getAscent() - fmAccel
				.getDescent()) / 2;
		...
		if (arrowIcon != null && arrowUsed) {
			arrowIconRect.x = acceleratorRect.x + maxAccWidth + 14
