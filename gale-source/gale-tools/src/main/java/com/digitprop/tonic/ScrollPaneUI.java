 ...
 * ------------------------------------------------------------------------
 */
public class ScrollPaneUI extends javax.swing.plaf.ScrollPaneUI implements
		ScrollPaneConstants {
	...
	/**
	 * Returns the preferred size of the specified component. In this case, this
	 * will be null to indicate to the LayoutManager that it must compute this
	 * value.
	 */
	...
	InputMap getInputMap(int condition) {
		if (condition == JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT) {
			InputMap keyMap = (InputMap) UIManager
					.get("ScrollPane.ancestorInputMap");
			InputMap rtlKeyMap;

			if (scrollpane.getComponentOrientation().isLeftToRight()
					|| ((rtlKeyMap = (InputMap) UIManager
							.get("ScrollPane.ancestorInputMap.RightToLeft")) == null)) {
				return keyMap;
			} else {
	...
	ActionMap createActionMap() {
		...
		map.put("scrollUp", new ScrollAction("scrollUp",
		...
		map.put("unitScrollUp", new ScrollAction("UnitScrollUp",
				SwingConstants.VERTICAL, -1, false));
		map.put("unitScrollDown", new ScrollAction("UnitScrollDown",
				SwingConstants.VERTICAL, 1, false));
		...
		if (scrollpane.getComponentOrientation().isLeftToRight()) {
			...
			map.put("scrollRight", new ScrollAction("ScrollRight",
					SwingConstants.HORIZONTAL, 1, true));
			map.put("unitScrollRight", new ScrollAction("UnitScrollRight",
					SwingConstants.HORIZONTAL, 1, false));
			map.put("unitScrollLeft", new ScrollAction("UnitScrollLeft",
					SwingConstants.HORIZONTAL, -1, false));
		} else {
			map.put("scrollLeft", new ScrollAction("scrollLeft",
					SwingConstants.HORIZONTAL, 1, true));
			...
			if (hsb != null) {
				...
				if (ltr) {
					...
					if (setValueCalled
						...
						value = Math.max(0,
								Math.min(max - extent, currentValue));
						/*
						...
						if (extent > max) {
							...
							value = Math.max(
									0,
									Math.min(max - extent, max - extent
											- viewPosition.x));
						}
					}
				}
	...
	/**
	 * Returns a shared <code>PropertyChangeListener</code> that will update the
	 * listeners installed on the scrollbars as the model changes.
	 */
			...
			if (vp != null && (view = vp.getView()) != null) {
				...
				if (view instanceof Scrollable) {
					if (block) {
						...
						amount = ((Scrollable) view)
								.getScrollableUnitIncrement(visRect,
										orientation, direction);
					}
				} else {
					if (block) {
