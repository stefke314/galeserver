...
public class AhaManager {
	...
	private String getHashString(String unhashed) {
		...
		try {
			...
			for (int i = 0; i < digest.length; i++) {
				byteHex = Integer.toHexString(digest[i] & 0xFF); // know no
																	// (unsigned)
																	// cast
				if (byteHex.length() == 1)
					hexdigest.append("0" + byteHex);
				else
					hexdigest.append(byteHex);
			}
			return hexdigest.toString();

		} catch (Exception e) {
			System.err
					.println("MD5 could not be instantiated, using standard hash which is XTREMELY UNSAFE");
	...
	public AhaManager(String l, String n) {
		login = l;
		name = n;
		passwd = null;
	}
